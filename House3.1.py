...
def get_cplus_2fa_code_from_zoho(max_wait=60):
    """
    從 Zoho Mail 讀取最新 Cplus 2FA code（只讀 Subject）
    Subject 格式：Cplus - XXXXXX is your verification code
    """
    load_dotenv()
    ...
    if not zoho_email or not zoho_password:
        logging.error("❌ 缺少 ZOHO_EMAIL 或 ZOHO_PASSWORD（請檢查 GitHub Secrets）")
        return None
    ...
    while time.time() - start_time < max_wait:
        try:
            ...
            # 搜尋 HIT 寄件人
            status, messages = mail.search(None, 'FROM "paklun@XXXXXX.com.hk"')
            if status == "OK" and messages[0]:
                ...
                # 提取 6 位數字 code
                match = re.search(r'Cplus - (\d{6}) is your verification code', subject)
                if match:
                    code = match.group(1)
                    logging.info(f"✅ 成功讀取 2FA Code: {code}")
                    mail.logout()
                    return code

            mail.logout()
        except Exception as e:
            logging.warning(f"讀取 Zoho Mail 失敗: {str(e)}")

        time.sleep(3)  # 每 3 秒檢查一次新 email

    logging.error("❌ 等待 2FA email 超時（60秒）")
    return None
